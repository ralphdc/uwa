	...
	<dd>
		<form id="formEdit" class="form" action="{-url:archive/edit_archive_do-}" method="post" enctype="multipart/form-data">
			...
			<div class="form-group">
				...
				<ul id='thematic_node_set' class="list-unstyled">
					{-foreach:$_AI['a_t_node'],$k,$atn-}<li k="{-:$k-}" class="">
						<input name="a_t_node[{-:$k-}][name]" size="20" value="{-:$atn['name']-}" class="control-input required" required="required" placeholder="{-:@NODE_NAME-}" />
						<input name="a_t_node[{-:$k-}][alias]" size="10" value="{-:$atn['alias']-}" class="control-input" placeholder="{-:@NODE_ALIAS-}" />
						<span class="btn choose_archive" to_id="node_archive_{-:$k-}">{-:@CHOOSE_ARCHIVE-}</span>
						<span class="btn delete">{-:@DELETE_NODE-}</span>
						<div class="archive_set" to_id="node_archive_{-:$k-}">
						{-if:!empty($atn['archive_set'])-}<uwa:a_list titlelen="20" aid="$atn.archive_set" key="ask">
							<span class="badge badge-primary" aid="{-:$item['archive_id']-}"><a href="{-:$item['a_url']-}" target="_blank" style="color:#fff">{-:$item['a_title']-}</a> <b>╳</b></span>
						</uwa:a_list>{-:/if-}
						</div>
