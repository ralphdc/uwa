...
<dl class="abox">
	<dt><strong>{-:@EDIT_CUSTOM_MODEL_CONTENT-}</strong></dt>
	<dd>
		<table class="formTable">
			...
			<tr>
				<td class="inputArea">
					<label><input type="radio" value="0" name="status"{-if:0==$_CMCI['status']-} checked="checked"{-:/if-}> {-:@NOT_PASSED-}</label>
					<label><input type="radio" value="1" name="status"{-if:1==$_CMCI['status']-} checked="checked"{-:/if-}> {-:@PASSED-}</label>
					<label><input type="radio" value="2" name="status"{-if:2==$_CMCI['status']-} checked="checked"{-:/if-}> {-:@REFUNDED-}</label>
				</td>
				<td class="inputArea">{-:$_CMCI['member_id']-}</td>
			</tr>
		</table>
	</dd>
</dl><!--/.atab-->
<div id="operation">
	...
	<input name="id" type="hidden" value="{-:$_CMCI['id']-}">
	<input name="cm_alias" type="hidden" value="{-:$_CMI['cm_alias']-}">
	<span class="btn_b submit" action="{-url:custom_model/edit_content_do-}" to="#formEdit">{-:@SUBMIT-}</span>
