		...
		<table class="formTable">
			...
			<tr>
				<td class="inputArea">
					...
					{-foreach:$_MSL,$ms-}
						<option {-if:$ms['ms_alias']==$_MI['ms_alias']-} selected="selected"{-:/if-} value="{-:$ms['ms_alias']-}">{-:$ms['ms_name']-}</option>
					{-:/foreach-}
					</select> <span class="fc_r">*</span> <span class="fc_gry">{-:@MENU_SPACE_TIP-}</span>
				</td>
				<td class="inputArea">
					<select name="m_parent_id" id="m_parent_id">
						<option {-if:0==$_MI['m_parent_id']-} selected="selected"{-:/if-} value="0">{-:@TOP_MENU-}</option>
					{-foreach:$_ML,$m-}
						{-if:$m['menu_id']!=$_MI['menu_id']-}
						<option {-if:$m['menu_id']==$_MI['m_parent_id']-} selected="selected"{-:/if-} value="{-:$m['menu_id']-}">{-:$m['m_name']-}</option>
						{-:/if-}
					{-:/foreach-}
					</select>
			...
			<tr>
				<td class="inputArea">
					<input id="m_name" class="required i" type="text" value="{-:$_MI['m_name']-}" name="m_name" maxlength="96" size="30"> <span class="fc_r">*</span> <span class="fc_gry">{-:@M_NAME_TIP-}</span>
				</td>
				<td class="inputArea">
					<input class="i" type="text" value="{-:$_MI['m_tip']-}" name="m_tip" id="m_tip" maxlength="255" size="50"> <span class="fc_gry">{-:@M_TIP_TIP-}</span>
				</td>
			</tr>
			<tr>
			...
			<tr>
				<td colspan="2" class="inputArea">
					<input class="required i" type="text" value="{-:$_MI['m_url']-}" name="m_url" id="m_url" maxlength="255" size="60"> <span class="fc_r">*</span> <span class="fc_gry">{-:@M_URL_TIP-}</span>
				</td>
			</tr>
			<tr>
			...
			<tr>
				<td class="inputArea">
					<label><input id="compose_btn" type="radio" onclick="$('#compose_url').show();" {-if:0==$_MI['m_type']-} checked="checked"{-:/if-} value="0" name="m_type"> {-:@COMPOSE-}</label>
					<label><input type="radio" onclick="$('#compose_url').hide();" {-if:1==$_MI['m_type']-} checked="checked"{-:/if-} value="1" name="m_type"> {-:@DIRECT-}</label> <span class="fc_r">*</span> <span class="fc_gry">{-:@M_TYPE_TIP-}</span>
				</td>
				<td class="inputTip">
					<input class="required i" type="text" value="{-:$_MI['m_display_order']-}" name="m_display_order" maxlength="20" size="10"> <span class="fc_r">*</span> <span class="fc_gry">{-:@M_DISPLAY_ORDER_TIP-}</span>
				</td>
			</tr>
			<tr id="compose_url"{-if:1==$_MI['m_type']-} style="display:none"{-:/if-}>
				<td colspan="2" class="inputArea">
			...
			<tr>
				<td class="inputArea">
					<input id="m_target" class="required i" type="text" name="m_target" size="20" value="{-:$_MI['m_target']-}">
					<select id="m_target_select" onchange="$('#m_target').val($('#m_target_select').val());">
						<option value="">{-:@SELECT-}</option>
						<option {-if:'_self'==$_MI['m_target']-} selected="selected"{-:/if-} value="_self">{-:@SELF_WINDOW-}</option>
						<option {-if:'_blank'==$_MI['m_target']-} selected="selected"{-:/if-} value="_blank">{-:@NEW_WINDOW-}</option>
						<option {-if:'_parent'==$_MI['m_target']-} selected="selected"{-:/if-} value="_parent">{-:@PARENT_FRAME-}</option>
						<option {-if:'_top'==$_MI['m_target']-} selected="selected"{-:/if-} value="_top">{-:@TOP_FRAME-}</option>
...
<div id="operation">
	...
	<input type="hidden" value="{-:$_MI['menu_id']-}" name="menu_id">
	<span class="btn_b submit" action="{-url:menu/edit_menu_do-}" to="#formEdit">{-:@SUBMIT-}</span>
