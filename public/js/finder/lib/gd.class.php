...
class gd {
	...
	protected function build_image($image) {
		...
		$return = (is_resource($image) && (get_resource_type($image) == "gd") && isset($width) && isset($height) && (preg_match('/^[1-9][0-9]*$/', $width) !== false) && (preg_match('/^[1-9][0-9]*$/', $height) !== false)) ? array(
			$image,
			$width,
			$height) : false;
	...
	public function __construct($image, $bigger_size = null) {
		...
		if(!is_null($this->image) && !is_null($bigger_size) && (preg_match('/^[1-9][0-9]*$/', $bigger_size) !== false)) {
			$image = $this->image;
	...
	public function resize($width, $height) {
		...
		return ((false !== ($img = new gd(array($width, $height)))) && $img->imagecopyresampled($this) && (false !== ($this->image = $img->get_image())) && (false !== ($this->width = $img->get_width())) && (false !== ($this->height = $img->get_height())));
	}
	...
	public function gd_color() {
		...
		elseif((count($args) == 3) && preg_match($expr_byte, $args[0]) && preg_match($expr_byte, $args[1]) && preg_match($expr_byte, $args[2])) {
			list($r, $g, $b) = $args;
		}
		else {
			return false;
	...
	public function imagecopy($src, $dst_x = 0, $dst_y = 0, $src_x = 0, $src_y = 0, $dst_w = null, $dst_h = null, $src_w = null, $src_h = null) {
		$image_details = $this->build_image($src);
	...
	public function imagecopyresampled($src, $dst_x = 0, $dst_y = 0, $src_x = 0, $src_y = 0, $dst_w = null, $dst_h = null, $src_w = null, $src_h = null) {
		$image_details = $this->build_image($src);
