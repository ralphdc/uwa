...
class uploader {
	 ...
	 * @var * array */
	protected $langInputNames = array(
		'lang',
		'langCode',
		'lng',
		'language',
	...
	public function __construct() {
		...
		// FULL URL
		if(preg_match('/^([a-z]+)\:\/\/([^\/^\:]+)(\:(\d+))?\/?(.+)\/?$/', $this->config['uploadURL'])) {
			//list($unused, $protocol, $domain, $unused, $port, $path) = $patt;
			extract(parse_url($this->config['uploadURL']));
			$path = path::normalize($path);
			$this->config['uploadURL'] = "$scheme://$host".(strlen($port) ? ":$port" : "").$path;
		...
		foreach($this->langInputNames as $key) {
			if(isset($this->get[$key]) && preg_match('/^[a-z][a-z\._\-]*$/i', $this->get[$key]) && file_exists(FINDER_PATH . D_S . "lang/" . strtolower($this->get[$key]) . ".php")) {
		...
		if(isset($this->config['check4htaccess']) && $this->config['check4htaccess']) {
	...
	protected function checkUploadedFile(array $aFile = null) {
		...
		// FILESIZE CHECK
		elseif($file['size'] > $config['maxFileSize']) {
			return $this->label("File size is out of limit.");
		}
	...
	protected function imageResize($image, $file = null) {
		...
		if((!$this->config['maxImageWidth'] && !$this->config['maxImageHeight']) || (($gd->get_width() <= $this->config['maxImageWidth']) && ($gd->get_height() <= $this->config['maxImageHeight']))) {
			return true;
		}
		...
		elseif(!$gd->resize_fit($this->config['maxImageWidth'], $this->config['maxImageHeight'])) {
			return false;
		}
	...
	protected function localize($langCode) {
		require FINDER_PATH . D_S . "lang/{$langCode}.php";
