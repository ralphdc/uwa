<?php /* @desc File related functionality */?>
	...
	setTimeout(function() {
		...
		$.each(browser.files, function(i, file) {
			...
			if(_.kuki.get('view') == 'list') {
				...
				if(file.thumb)
					...
					icon = '.';
				icon = '<?php echo __FINDER__; ?>themes/' + browser.theme + '/img/files/small/' + icon + '.png';
				html += '<tr class="file">' +
				...
				if(file.thumb)
					...
					if(!icon.length) icon = '.';
					icon = '<?php echo __FINDER__; ?>themes/' + browser.theme + '/img/files/big/' + icon + '.png';
				}
				html += '<div class="file">' +
		...
		$.each(browser.files, function(i, file) {
			...
			if(_.inArray(file.name, selected) ||
				((typeof selected != 'undefined' && null != selected) && !selected.push && (file.name == selected))
			)
...
browser.returnFile = function(file) {
	...
	if(this.opener.CKEditor) {
		...
		if(typeof(win.ImageDialog) != "undefined" && null != win.ImageDialog) {
