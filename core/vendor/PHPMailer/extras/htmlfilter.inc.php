...
function HTMLFilter($body, $trans_image_path, $block_external_images = false) {
	...
	$bad_attvals = Array("/.*/" => Array(
			...
			"/^style/i" => Array(Array(
					...
					"/(.*)\s*:\s*url\s*\(\s*([\'\"]*)\s*\S+script\s*:.*([\'\"]*)\s*\)/si"), Array(
					"idiocy",
					"idiocy",
					"idiocy",
					"idiocy",
	...
	if($block_external_images) {
		array_push($bad_attvals{'/.*/'} {
			'/^src|background/i'}
		[0], '/^([\'\"])\s*https*:.*([\'\"])/si');
		array_push($bad_attvals{'/.*/'} {
			'/^src|background/i'}
		[1], "\\1$trans_image_path\\1");
		array_push($bad_attvals{'/.*/'} {
			'/^style/i'}
		[0], '/url\(([\'\"])\s*https*:.*([\'\"])\)/si');
		array_push($bad_attvals{'/.*/'} {
			'/^style/i'}
		[1], "url(\\1$trans_image_path\\1)");
	}
